...
data class Outcome<T>(
    ...
    companion object {
        ...
        fun <T> fail(e: Exception, type: OutcomeType? = null, networkType: NetworkErrorType? = null, data: T? = null): Outcome<T> {
            return Outcome(success = false, message = "", outcomeType = type, networkType = networkType, data = data)
        }

        fun error(e: Exception, type: OutcomeType? = null, networkType: NetworkErrorType? = null): Outcome<*> {
            return Outcome<Any>(success = false, message = "", outcomeType = type, networkType = networkType)
        }

        fun <T> fail(e: Throwable, type: OutcomeType? = null, networkType: NetworkErrorType? = null, data: T? = null): Outcome<T> {
            return Outcome(success = false, message = "displayMessage", outcomeType = type, networkType = networkType, data = data)
        }
